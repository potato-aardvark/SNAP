    ...
    def onclick(event):
        ...
        if keep == 'y' or keep == "":
            ...
            if len(x_cens) == 1:
                ...
                use_prev = eval(input("Use previous photometry parameters? (y/n)"))
            ...
            if use_prev == 'y' or use_prev == "":
                ...
                aper = eval(input("Aperture=? (float / empty for PSF)"))
                if aper == "":
                    #PSF photometry parameters
                    psf = eval(input("PSF type=? ('1', '2', '3', 's<sersic index>')"))
                    apers.append(None)
                    psfs.append(psf)
                else:
                ...
                fit_sky = eval(input("Fit planar background? (y/n)"))
